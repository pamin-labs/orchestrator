 ...
 */
/**
 * Two surfaces, because they are two mechanisms: the panel's catalogs are
 * generated by `lingui extract`, while `lang.ts` is *allowed* to be partial — an
 * untranslated key there falls back to the English row, which is a sentence in
 * the other language rather than a build failure. Both happen to be complete
 * right now. Only the panel is per-language; `lang.ts` speaks Chinese and
 * English only.
 */
...
const COPY = {
  "README.md": {
    ...
    orchestrator: (n: number) =>
      `**What the orchestrator says through \`say()\`** · ${n}, English and one Chinese table — a Traditional reader gets these in Simplified`,
  },
  "README.zh-CN.md": {
    head: ["语言", "进度", "条数"],
    source: "源语言",
    panel: (n: number) => `**面板** · ${n} 条消息`,
    orchestrator: (n: number) => `**编排器经 \`say()\` 说的话** · ${n} 条，只有中英两套 —— 繁体读者拿到的是简体`,
  },
} as const;
