...
/**
 * Checked in and regenerated by `i18n:extract`, not pushed back by CI.
 *
 * That job needs `contents: write` and a checkout that keeps its credentials —
 * on the one running a pull request's own code. A fork's token is read-only
 * whatever the block says, so it reddens every outside contribution; and a push
 * made with `GITHUB_TOKEN` triggers no workflow, so the auto-commit lands at a
 * head where a required check never reports.
 */
/**
