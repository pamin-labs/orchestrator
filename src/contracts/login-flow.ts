 ...
 * snapshot is the fleet's state, re-read on every change, and this is a
 * short-lived handshake that exists between clicking 连接 and the CLI coming
 * back. They were in one file only because both are things the panel receives.
 *
 * The seam it does cross is real, which is why it stays a contract rather than
 * moving into either side: the server produces the value and the browser
 * validates it, so the schema and the type are two views of one wire format and
 * splitting them would let the two drift with nothing to notice.
 */
