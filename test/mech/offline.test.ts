...
async function seed(): Promise<{ db: DB; ctx: Ctx; sched: Scheduler; ran: Job[]; online: { v: boolean } }> {
  const db = await openMemory();
  const f = fx.on(db);
  const p = await f.project.create({ name: "p" });
  const g = await f.runningGrp.create({ project_id: p.id, name: "g1" });
  await f.agent.create({ project_id: p.id, grp_id: g.id, runtime: "claude", token: "tok" });
...
const stateOf = async (db: DB, id: number) =>
  (await db.select({ state: job.state }).from(job).where(eq(job.id, id)))[0]!.state;

test("offline holds agent turns in the queue instead of dispatching them", async () => {
  const h = await seed();
  const id = await h.sched.enqueue("agent_turn", { grp_id: 1, agent_id: 1, payload: { role: "engineer" } });
  ...
  expect(await stateOf(h.db, id)).toBe("pending");
...
test("a turn in flight is re-queued and its requirement is left running", async () => {
  const h = await seed();
  const id = await h.sched.enqueue("agent_turn", { grp_id: 1, agent_id: 1, payload: { role: "engineer" } });
  await h.db.update(job).set({ state: "running", started_at: 0 }).where(eq(job.id, id));
  await h.db.update(agent).set({ state: "running" }).where(eq(agent.id, 1));

  const requeued = await holdForOffline(h.ctx, 1000);
  expect(requeued).toBe(1);
  expect(await stateOf(h.db, id)).toBe("cancelled");
  ...
  expect((await h.db.select({ status: grp.status }).from(grp).where(eq(grp.id, 1)))[0]!.status).toBe("RUNNING");
  // The agent is idle again, or everything downstream skips it forever.
  expect((await h.db.select({ state: agent.state }).from(agent).where(eq(agent.id, 1)))[0]!.state).toBe("idle");
  const back = await h.db
...
test("a turn the network killed does not spend its one retry", async () => {
  const h = await seed();
  ...
  const already: Job = {
    ...
    agent_id: 1,
    slice_id: null,
    payload_json: { role: "engineer", resumed: true },
    payload: { role: "engineer", resumed: true },
    priority: 0,
    state: "cancelled",
    error: "offline: the host lost its network",
  };
  expect(await resumeReclaimed(h.sched, [already])).toBe(1);

  // And the exemption is exactly that error, not any cancellation.
  expect(await resumeReclaimed(h.sched, [{ ...already, error: "interrupted (keep)" }])).toBe(0);
});

test("a reclaim re-queues lease, gate and reconcile work, never the free kinds", async () => {
  ...
  expect(await resumeReclaimed(h.sched, [lease, gate, reconcile])).toBe(3);
  expect(await resumeReclaimed(h.sched, [watchdog])).toBe(0);

  const requeued = await h.db
    .select({ kind: job.kind, payload_json: job.payload_json })
    .from(job)
    .where(eq(job.state, "pending"))
    .orderBy(asc(job.id));
  ...
  for (const r of requeued) ResumedPayload.parse(r.payload_json);
  expect(ResumedPayload.parse(requeued[0]!.payload_json).lease_id).toBe(7);
});
...
test("no container to open holds every turn instead of failing each group once", async () => {
  ...
  await ensureSandbox(ctx, { grp: 1 }).catch(() => {});
  expect((await h.db.select({ n: count() }).from(event).where(eq(event.kind, "escalation")))[0]!.n).toBe(1);
  resetSandboxHold();
});
