...
function seed(): { db: DB; sched: Scheduler; ran: Job[] } {
  ...
  db.run("INSERT INTO project (name, repo_path, remote, created_at) VALUES ('p', '/tmp/p', 'git@github.com:me/x.git', 0)");
  // A second project, on a different repository, whose credential is fine.
  db.run("INSERT INTO project (name, repo_path, remote, created_at) VALUES ('q', '/tmp/q', 'git@github.com:me/y.git', 0)");
...
const answer = (status: number, body: unknown = {}): Fetcher => async () =>
...
test("a boss-bucket failure holds that project's turns and leaves the other project alone", async () => {
  const h = seed();
  await makeGithub(h.db, answer(401, { message: "Bad credentials" })).request("GET", "/repos/me/x/pulls/7");
  ...
  const state = (id: number) => h.db.query<{ state: string }, [number]>("SELECT state FROM job WHERE id = ?").get(id)!.state;
...
test("a 502 does not hold — that is what backoff is for", async () => {
  const h = seed();
  const r = await makeGithub(h.db, answer(502, { message: "bad gateway" })).request("GET", "/repos/me/x/pulls/7");
...
test("the next success clears the hold, with nobody clicking anything", async () => {
  const h = seed();
  await makeGithub(h.db, answer(404, { message: "Not Found" })).request("GET", "/repos/me/x/pulls/7");
  expect(repoHeld(h.db, 1)).toBe(true);
  expect(openEscalations(h.db)).toHaveLength(1);

  await makeGithub(h.db, answer(200, { number: 7 })).request("GET", "/repos/me/x/pulls/7");
  expect(repoHeld(h.db, 1)).toBe(false);
...
test("one escalation for the project, however many groups run into it", async () => {
  ...
  for (let i = 0; i < 5; i++) await gh.request("GET", `/repos/me/x/pulls/${i}`);
  ...
  await makeGithub(h.db, answer(401, { message: "Bad credentials" })).request("GET", "/repos/me/y/pulls/1");
...
test("a project that recovers and breaks again can warn a second time", async () => {
  ...
  await bad.request("GET", "/repos/me/x/pulls/7");
  await makeGithub(h.db, answer(200, { number: 7 })).request("GET", "/repos/me/x/pulls/7");
  expect(openEscalations(h.db)).toHaveLength(0);
