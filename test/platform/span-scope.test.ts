...
/**
 * The aggregation is SQL, so it is tested against a real database rather than
 * against a reimplementation of it in the test.
 *
...
async function write(db: DB, rows: (Partial<SpanRow> & { attributes: Record<string, unknown> })[]): Promise<void> {
  await writeSpans(db, rows.map(span));
}

const inProject = { "project.id": 7, "grp.id": 3 };
const otherProject = { "project.id": 8, "grp.id": 4 };

test("a p95 is the p95, and a p50 is the p50", async () => {
  ...
  const [stage] = await stageStats(db, { kind: "group", id: 3 }, recentWindow(600_000, NOW));
...
test("one sample is its own p50 and p95 rather than a null", async () => {
  const db = await openMemory();
  await write(db, [{ name: "sandbox.create", durationMs: 4200, attributes: inProject }]);
...
test("a project's spans include the ones that only name its group", async () => {
  ...
  const db = await openMemory();
  await fx.on(db).project.create({ name: "mine" });
  await fx.on(db).project.create({ name: "theirs" });
  await fx.on(db).runningGrp.create({ project_id: 1, name: "g1" });
  await fx.on(db).runningGrp.create({ project_id: 2, name: "g2" });
  await write(db, [
  ...
  const mine = await stageStats(db, { kind: "project", id: 1 }, recentWindow(600_000, NOW));
  ...
  const theirs = await stageStats(db, { kind: "project", id: 2 }, recentWindow(600_000, NOW));
  expect(theirs.map((s) => s.name)).toEqual(["turn.provider"]);
  expect(theirs[0]?.totalMs).toBe(100);
});

test("a group with no project row does not fall into some other project", async () => {
  const db = await openMemory();
  await fx.on(db).project.create({ name: "mine" });
  ...
  await write(db, [{ name: "turn", durationMs: 40, attributes: { "grp.id": 999 } }]);
  expect(await stageStats(db, { kind: "project", id: 1 }, recentWindow(600_000, NOW))).toEqual([]);
});

test("a scope filter excludes another project's spans", async () => {
  const db = await openMemory();
  await write(db, [
  ...
  const mine = await stageStats(db, { kind: "project", id: 7 }, recentWindow(600_000, NOW));
  expect(mine).toHaveLength(1);
  expect(mine[0]?.count).toBe(1);
  expect(mine[0]?.totalMs).toBe(100);

  const theirs = await stageStats(db, { kind: "project", id: 8 }, recentWindow(600_000, NOW));
  expect(theirs[0]?.totalMs).toBe(900);
});

test("a group scope excludes a sibling group inside the same project", async () => {
  const db = await openMemory();
  await write(db, [
  ...
  expect((await stageStats(db, { kind: "group", id: 3 }, recentWindow(600_000, NOW)))[0]?.totalMs).toBe(100);
  expect((await stageStats(db, { kind: "group", id: 5 }, recentWindow(600_000, NOW)))[0]?.totalMs).toBe(900);
  // The project sees both, which is the point of having the two scopes.
  expect((await stageStats(db, { kind: "project", id: 7 }, recentWindow(600_000, NOW)))[0]?.count).toBe(2);
});

test("a span with no scope is counted in the system view and nowhere else", async () => {
  const db = await openMemory();
  await write(db, [
  ...
  const system = await stageStats(db, { kind: "system" }, recentWindow(600_000, NOW));
  ...
  expect((await stageStats(db, { kind: "project", id: 7 }, recentWindow(600_000, NOW))).map((s) => s.name)).toEqual([
    "turn",
  ]);
  expect((await stageStats(db, { kind: "group", id: 3 }, recentWindow(600_000, NOW))).map((s) => s.name)).toEqual([
    "turn",
  ]);
});

test("a span carrying a group but no project is system work in neither view", async () => {
  const db = await openMemory();
  ...
  await write(db, [{ name: "job.reconcile", durationMs: 5, attributes: { "grp.id": 3 } }]);

  expect(await stageStats(db, { kind: "system" }, recentWindow(600_000, NOW))).toHaveLength(0);
  expect((await stageStats(db, { kind: "group", id: 3 }, recentWindow(600_000, NOW)))[0]?.name).toBe("job.reconcile");
});

test("errors are counted per stage without being dropped from the timing", async () => {
  const db = await openMemory();
  await write(db, [
...
test("the window excludes what is older than it", async () => {
  const db = await openMemory();
  await write(db, [
  ...
  expect((await stageStats(db, { kind: "group", id: 3 }, recentWindow(60_000, NOW)))[0]?.count).toBe(1);
  ...
  expect(await stageStats(db, { kind: "group", id: 3 }, { from: NOW - 60_000, to: NOW - 40_000 })).toEqual([]);
  expect((await stageStats(db, { kind: "group", id: 3 }, recentWindow(24 * 60 * 60 * 1_000, NOW)))[0]?.count).toBe(2);
});

test("stages are ordered by what they cost, so the expensive one reads first", async () => {
  const db = await openMemory();
  await write(db, [
...
test("a trace's span is measured across the scope, not taken from a root", async () => {
  const db = await openMemory();
  ...
  const [trace] = await traceList(db, { kind: "group", id: 3 }, 20, recentWindow(600_000, NOW));
...
test("a trace is failed when any span in it failed", async () => {
  const db = await openMemory();
  await writeSpans(db, [
  ...
  expect((await traceList(db, { kind: "group", id: 3 }, 20, recentWindow(600_000, NOW)))[0]?.failed).toBe(true);
});

test("traces come back newest first and no more than the limit", async () => {
  const db = await openMemory();
  await writeSpans(
    db,
  ...
  const traces = await traceList(db, { kind: "group", id: 3 }, 3, recentWindow(600_000, NOW));
  expect(traces).toHaveLength(3);
  expect(traces.map((t) => t.startedAt)).toEqual([NOW - 46_000, NOW - 47_000, NOW - 48_000]);
});

test("the trend buckets by time and takes percentiles of whole traces", async () => {
  const db = await openMemory();
  ...
  const points = await trend(db, { kind: "group", id: 3 }, hour, recentWindow(6 * hour, NOW));
...
test("a scope with nothing in it reports nothing rather than failing", async () => {
  const db = await openMemory();
  const empty: ReadScope = { kind: "project", id: 404 };
  expect(await stageStats(db, empty, recentWindow(600_000, NOW))).toEqual([]);
  expect(await traceList(db, empty, 20, recentWindow(600_000, NOW))).toEqual([]);
  expect(await trend(db, empty, 60_000, recentWindow(600_000, NOW))).toEqual([]);
});

// ── folded stacks, the flamegraph's data ───────────────────────────────────

test("call paths are summed across every trace in the scope", async () => {
  const db = await openMemory();
  ...
  expect(await foldedStacks(db, { kind: "group", id: 3 }, recentWindow(600_000, NOW))).toEqual([
    { path: "turn", totalMs: 200, count: 2 },
    { path: "turn;turn.provider", totalMs: 160, count: 2 },
  ]);
});

test("a root is a span whose parent is outside the scope, not one with no parent", async () => {
  const db = await openMemory();
  ...
  expect((await foldedStacks(db, { kind: "group", id: 3 }, recentWindow(600_000, NOW))).map((row) => row.path)).toEqual(
    ["turn", "turn;turn.provider"],
  );
});

test("span ids are only unique within a trace, and the walk respects that", async () => {
  const db = await openMemory();
  ...
  const paths = (await foldedStacks(db, { kind: "group", id: 3 }, recentWindow(600_000, NOW))).map((row) => row.path);
  expect(paths.toSorted()).toEqual(["root530", "root530;leaf", "root531", "root531;leaf"]);
});

test("the scope filter applies to the walk, not only to its roots", async () => {
  const db = await openMemory();
  await writeSpans(db, [
...
test("a cycle in the parent ids terminates, and still counts what it cost", async () => {
  const db = await openMemory();
  await writeSpans(db, [
  ...
  await db
    .update(spanTable)
    .set({ parent_span_id: spanId(553) })
    .where(eq(spanTable.span_id, spanId(552)));

  const rows = await foldedStacks(db, { kind: "group", id: 3 }, recentWindow(600_000, NOW));
...
test("a deep trace is walked, and the path names every level of it", async () => {
  const db = await openMemory();
...
test("a scope with nothing in it folds to nothing", async () => {
  expect(await foldedStacks(await openMemory(), { kind: "system" }, recentWindow(600_000, NOW))).toEqual([]);
});

// ── a requirement, split by slice ──────────────────────────────────────────

test("a requirement's time is split by the slice that spent it", async () => {
  const db = await openMemory();
  await write(db, [
  ...
  expect(await sliceCosts(db, 3, recentWindow(600_000, NOW))).toEqual([
...
test("a requirement that has run nothing splits into nothing", async () => {
  expect(await sliceCosts(await openMemory(), 3, recentWindow(600_000, NOW))).toEqual([]);
});

test("the data extent is where the spans are, not where the query looked", async () => {
  const db = await openMemory();
  await write(db, [
  ...
  const extent = await spanExtent(db, { kind: "project", id: 7 }, NOW);
...
test("a scope with no spans has no extent, which is not a window of zero width", async () => {
  const db = await openMemory();
  expect(await spanExtent(db, { kind: "group", id: 3 }, NOW)).toBeNull();

  // And one span is widened to something a range can be clamped inside: `from`
  // and `to` equal is not a window anything fits in.
  await write(db, [{ name: "turn", startedAt: NOW - 1_000, durationMs: 0, attributes: inProject }]);
  const one = await spanExtent(db, { kind: "project", id: 7 }, NOW);
