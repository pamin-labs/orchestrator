...
export function projectRow(st: State, id: number) {
  ...
  return {
    ...
    bits: [
      count(w.cards.length, i18n.t("home.model.cardsPending", "张卡待批")),
      count(w.slices.length, i18n.t("home.model.slicesPending", "片待查收")),
      count(w.merges.length, i18n.t("home.model.mergesPending", "个待合入")),
      count(w.asks.length, i18n.t("home.model.asksPending", "个提问")),
    ].filter(Boolean),
    live: gs.filter((g) => LIVE.has(g.status)).map((g) => g.name),
    meta: [count(gs.length, i18n.t("home.model.groupCount", "个需求")), tokens ? `${K(tokens)} tokens` : ""].filter(
      Boolean,
    ),
  };
}
