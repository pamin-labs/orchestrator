  ...
  return (
    <Pane className="@container">
      <div className="grid max-w-[72rem] grid-cols-1 gap-x-12 gap-y-9 @3xl:grid-cols-2">
        {RUNTIMES.map((r) => (
          <section key={r.key}>
            <H2 className="mb-1.5">{r.label}</H2>
            <Credential
              runtime={r}
        ...
        <section>
          <H2 className="mb-1.5">环境</H2>
          <FieldGroup className="[--label:9rem]">
            {checks
              .filter((c) => !isCredential(c))
              .map((c) => (
                <Field key={c.name} data-invalid={!c.ok} className="py-1.5">
                  <FieldTitle>
                    {c.ok ? (
...
function Credential(props: {
  ...
  return (
    <FieldGroup className="[--label:4.5rem]">
      {/* What is stored, on the line the section is named by. */}
      <Field className="py-1.5" data-invalid={!cur}>
        <FieldTitle className="text-ink-3">现在</FieldTitle>
