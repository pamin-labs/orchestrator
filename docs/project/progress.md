...
- **The panel reads in English or Chinese, and the compiler owns the ids.** 723
  ...
  `README`'s translation table is generated by `bun run i18n:progress` and
  checked in CI. Which language the panel speaks follows `output.language` with
  a per-browser override in Settings → 外观. ADR 041 has the Bun findings, the
  babel boundary against ADR 015, and the measured cost: `bun run test` +10.5%
  CPU, `build:web` 0.22s → 0.25s, bundle 1.70MB → 1.86MB.
