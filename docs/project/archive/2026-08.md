# Project progress archive — August 2026

What `docs/project/progress.md` accumulated between 2026-08-13 and 2026-08-30,
moved here verbatim so the active file could go back to being a snapshot.
...
ADRs, and a governance test caps the active file's length. This directory exists
because three ADRs and a test comment cite entries below as evidence, and a
citation that resolves to nothing is worse than no citation.

Read it oldest-first, the order it was written in. Superseded notes are left in
place, including the text they supersede — a correction that deletes what it
corrects hides the thing worth knowing.
...
## The last three entries, which had been written above the title

These were prepended above `# Project progress` rather than appended below it,
so the file was growing from both ends at once. They are the newest three, and
they belong here, at the end of the sequence, in the shape they were written.
...
## The current-state sections as they stood on 2026-08-30

The snapshot this archive was split out of, kept because three of its "next
executable items" were finished and are cited as evidence, and because its
blockers record what was resolved rather than what is open. The active file
carries the re-measured versions of these.
