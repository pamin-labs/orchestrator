...
| file-access-to-http (CodeQL) | `src/api/panel/authflow.ts` probe | **fixed** | the sink is the header, not the URL: on the `adopt` path the secret is read out of the sandbox server's own TOML and put on the wire, to an address a settings knob chooses. Adopting now returns before the probe rather than joining it through a shared `auth` binding — a `!("adopt" in b)` guard alone left both branches meeting in one variable, which is what the analysis follows and one refactor from being reachable again. Nothing is sent: a key read from a server's own config *is* the key that server runs with. It is stored bound to the address in that same file | api |
