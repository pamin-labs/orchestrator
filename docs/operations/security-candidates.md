...
| file-access-to-http (CodeQL) | `src/api/panel/authflow.ts:142` | **fixed** | the sink is the header, not the URL: on the `adopt` path the secret is read out of the sandbox server's own TOML and put on the wire, to an address chosen by a settings knob. The probe is now skipped for an adopted key — a key read from a server's own config *is* the key that server runs with, so asking it to confirm its own file answers nothing — and the key is bound to the address in that same file. No file value reaches a request | api |
