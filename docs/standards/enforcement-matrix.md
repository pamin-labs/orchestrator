...
| Semantic/cross-language retrieval | **nobody — refused, measured** | none; the gap is stated | `@huggingface/transformers` at 384 MB against a 160 MB archive budget, and `@orama/plugin-embeddings` at 69 MB which ships no TensorFlow backend and cannot load one. See [ADR 031](../adr/031-embeddings-do-not-fit-in-this-binary.md) | a JS embedding runtime measured in tens of megabytes, or the boss asks for hosted embeddings explicitly |
