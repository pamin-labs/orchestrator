...
**Settings pane 拆分进行中：** 账号表单、Claude setup-token 与 Codex device flow 只有 `web/src/views/settings/credentials.tsx` 一份实现；GitHub 连接、安装账号和 commit trailer 控制只有 `web/src/views/settings/github.tsx` 一份实现。dialog shell 持有 `auth`/`gh` query、条件挂载、2/3 秒轮询、5 分钟截止与 cache invalidation；pane 只启动动作并请求刷新。`test/settings-boundary.test.ts` 守住 endpoint、DOM ID 和协调边界。TypeScript、oxlint、web build、88 个定向 checks 及完整测试全绿。

**`bun test` 705 checks 绿（6 skip 是要真沙盒服务器的）。** `bun run dev`（构建前端 + 起服务），web 在 `http://127.0.0.1:47821`。
