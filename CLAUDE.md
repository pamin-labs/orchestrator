...
bun run test                 # 测试套件（限并发，避免 git-heavy tests 争抢进程）
...
**业务逻辑写对只是第一道闸。新增手写逻辑前，先证明现有实现、标准库/平台、已装依赖和成熟热门开源方案不能覆盖；收尾必须同时检查删减空间、官方用法、重复实现和依赖合理性，缺一项都不算完成。交付前过四问：**

1. **能不能删？** 一次性 wrapper、只服务一个实现的接口、为了压指标拆出的 view model，都先收回；拆完比原来更长、更绕，不叫简化。
2. **是不是主流惯用法？** 网络、校验、构建、CI、发布依次检查标准库/平台原生能力、当前框架的官方主路径、已装依赖，以及成熟且持续维护的热门开源方案；能让 Hono RPC、Zod、Bun、GitHub Action 或既有库自己保证的，不在旁边再包一套协议。
3. **同一事实有没有第二份？** schema、状态清单、错误解释、生命周期、配置读取只能有一个 owner；两个调用方复制同一段，不等事故发生，先收进已有的共享层。
4. **为什么还要自己写？** 先证明现有实现、标准库、平台/框架官方能力、已装依赖和成熟热门开源方案都不能正确覆盖，再允许新增手写逻辑；交付说明要说清它们缺的那条项目性质。答不出来，默认是重复造轮子；反过来，只是 npm 上有一个包、却没有维护度/采用度或不值得依赖成本，也不是加依赖的理由。

验收看的是代码和证据：`rg`/import graph 证明没有旁路，官方文档或已装源码证明用法，`fallow audit` 找重复和复杂度，测试钉业务性质。工具建议也不是结论——`fix` 如果只会生成一大片 ignore，应该修代码或修入口，不能把报警器关掉。
